...
Dans le dernier Lab, nous avons combiné NPU et cloud pour réaliser une extension Visual Studio Code pour l'agent Phi3.
...
L'API Visual Studio Code Chat n'a pas encore été intégrée à l'API de code, mais est maintenant ajoutée via des extensions.
...
// Importer le module et le référencer avec l'alias vscode dans votre code ci-dessous
...
export function activate(extcontext: vscode.ExtensionContext) {
	...
	const phi3handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<IPHI3ChatResult> => {
		...
		if (request.command == 'gen') {
			...
			const content = "Bienvenue sur Phi-3 pour générer du code";
		...
		if (request.command == 'img') {
			...
			const content = "Bienvenue sur Phi-3 vision pour générer du code à partir d'une image";
