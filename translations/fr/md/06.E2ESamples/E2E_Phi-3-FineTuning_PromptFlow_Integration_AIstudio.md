...
Cet exemple de bout en bout (E2E) est basé sur le guide "[Fine-Tune and Integrate Custom Phi-3 Models with Prompt Flow in Azure AI Studio](https://techcommunity.microsoft.com/t5/educator-developer-blog/fine-tune-and-integrate-custom-phi-3-models-with-prompt-flow-in/ba-p/4191726?WT.mc_id=aiml-137032-kinfeylo)" de la communauté technique de Microsoft. Il présente les processus d'affinage, de déploiement et d'intégration des modèles Phi-3 personnalisés avec Prompt Flow dans Azure AI Studio. Contrairement à l'exemple E2E, "[Fine-Tune and Integrate Custom Phi-3 Models with Prompt Flow](./E2E_Phi-3-FineTuning_PromptFlow_Integration.md)", qui impliquait l'exécution de code localement, ce tutoriel se concentre entièrement sur l'affinage et l'intégration de votre modèle au sein d'Azure AI / ML Studio.

## Aperçu

Dans cet exemple E2E, vous apprendrez à affiner le modèle Phi-3 et à l'intégrer avec Prompt Flow dans Azure AI Studio. En exploitant Azure AI / ML Studio, vous établirez un workflow pour déployer et utiliser des modèles d'IA personnalisés. Cet exemple E2E est divisé en trois scénarios :
...
1. **[Scénario 1 : Configurer les ressources Azure et se préparer à l'affinage](../../../../md/06.E2ESamples)**
...
4. Effectuez les tâches suivantes :
    ...
    ![Remplissez azure machine learning.](../../../../translated_images/01-03-fill-AZML.a3f644231a76859c7d92134b7dea1dd0d4df1c11cc615351c95be5a2c7056b03.fr.png)
...
> [!NOTE]
>
> Seuls les abonnements Pay-As-You-Go (le type d'abonnement standard) sont éligibles pour l'allocation de GPU ; les abonnements à avantages ne sont actuellement pas pris en charge.
>
...
Pour affiner et déployer vos modèles, vous devez d'abord créer une identité gérée assignée par l'utilisateur (UAI) et lui attribuer les autorisations appropriées. Cette UAI sera utilisée pour l'authentification pendant le déploiement.
...
1. Naviguez vers la ressource d'identité gérée que vous avez créée.
...
1. Dans la page Ajouter une attribution de rôle, effectuez les tâches suivantes :
    ...
    ![Remplissez le rôle de contributeur.](../../../../translated_images/03-04-fill-contributor-role.8bc54b3ac8f64842c82b3379f3c3e9f8d778abf28c00e5b7b471935a86d3ae64.fr.png)
...
1. Effectuez les tâches suivantes pour naviguer vers la page Ajouter une attribution de rôle :
    ...
    - Naviguez vers le compte de stockage Azure que vous avez créé.
...
1. Dans la page Ajouter une attribution de rôle, effectuez les tâches suivantes :
    ...
    - Dans la page Membres, sélectionnez **Attribuer l'accès à** **Managed identity**.
    ...
    - Dans la page Sélectionner des identités gérées, sélectionnez l'**Identité gérée** sur **Managed Identity**.
    ...
    ![Sélectionnez l'identité gérée.](../../../../translated_images/03-08-select-managed-identity.f9a44699bf247a4583e2d377e304de8c3d8a602b7d3fed52b9ae790e64e94fe9.fr.png)
...
Pour télécharger les jeux de données nécessaires à l'affinage, vous configurerez un environnement local.
...
1. Ajoutez le code suivant dans le fichier *download_dataset.py*.
    ...
    def load_and_split_dataset(dataset_name, config_name, split_ratio):
        ...
        print(f"Taille du jeu de données original : {len(dataset)}")
        
        # Diviser le jeu de données en ensembles d'entraînement et de test (80%
1. Visitez [Azure ML Studio](https://ml.azure.com/home?wt.mc_id=studentamb_279723).
...
1. Effectuez les tâches suivantes :
    ...
    - Entrez le **Nom de l'ordinateur**. Il doit être une valeur unique.
...
1. Effectuez les tâches suivantes :
    ...
    ![Remplir la page d'ajustement.](../../../../translated_images/06-07-fill-finetuning.c76431cc247b6398fb9d33da62841adf87d5eebaa92cd79e80bd7bcbed49f1d3.fr.png)

    > [!TIP]
    >
    > Vous pouvez sélectionner **Advanced settings** pour personnaliser des configurations telles que **learning_rate** et **lr_scheduler_type** afin d'optimiser le processus d'ajustement selon vos besoins spécifiques.

1. Sélectionnez **Finish**.

1. Dans cet exercice, vous avez réussi à ajuster le modèle Phi-3 en utilisant Azure Machine Learning. Veuillez noter que le processus d'ajustement peut prendre un temps considérable. Après avoir lancé le travail d'ajustement, vous devez attendre qu'il se termine. Vous pouvez surveiller l'état du travail d'ajustement en naviguant vers l'onglet Jobs sur le côté gauche de votre espace de travail Azure Machine Learning. Dans la prochaine série, vous déploierez le modèle ajusté et l'intégrerez avec Prompt flow.

    ![Voir le travail d'ajustement.](../../../../translated_images/06-08-output.9f9cf6f9e9e83533b793a5ff3066b09475e299999fead6f98dfb102f48db0061.fr.png)

### Déployer le modèle Phi-3 ajusté

Pour intégrer le modèle Phi-3 ajusté avec Prompt flow, vous devez déployer le modèle pour le rendre accessible pour l'inférence en temps réel. Ce processus implique l'enregistrement du modèle, la création d'un point de terminaison en ligne et le déploiement du modèle.
...
1. Vous pouvez voir votre modèle enregistré en naviguant vers le menu **Models** dans l'onglet de gauche.
...
1. Effectuez les tâches suivantes :
    ...
    - Sélectionnez **Endpoint** sur **New** pour créer un point de terminaison.
    - Entrez le **Nom du point de terminaison**. Il doit être une valeur unique.
    - Entrez le **Nom du déploiement**. Il doit être une valeur unique.
...
1. Entrez le **Nom du projet**. Il doit être une valeur unique.
...
Pour intégrer votre modèle Phi-3 personnalisé avec Prompt flow, vous devez enregistrer le point de terminaison et la clé du modèle dans une connexion personnalisée. Cette configuration assure l'accès à votre modèle Phi-3 personnalisé dans Prompt flow.
...
1. Effectuez les tâches suivantes :
    ...
    - Après avoir ajouté les clés, sélectionnez **is secret** pour éviter que la clé ne soit exposée.
...
Vous avez ajouté une connexion personnalisée dans Azure AI Studio. Maintenant, créons un flux de prompt en suivant les étapes ci-dessous. Ensuite, vous connecterez ce flux de prompt à la connexion personnalisée afin de pouvoir utiliser le modèle ajusté dans le flux de prompt.

1. Accédez au projet Azure AI Studio que vous avez créé.

1. Sélectionnez **Flux de prompt** dans l'onglet à gauche.

1. Sélectionnez **+ Créer** dans le menu de navigation.

    ![Sélectionner Flux de prompt.](../../../../translated_images/08-12-select-promptflow.5e0527f1e5102c604e0e8a34f2321e28f8c815ec2908ae7038f012a088ff2bbc.fr.png)
...
#### Configurer le Flux de Prompt pour discuter avec votre modèle Phi-3 personnalisé

Vous devez intégrer le modèle Phi-3 ajusté dans un flux de prompt. Cependant, le flux de prompt existant fourni n'est pas conçu pour cela. Vous devez donc redessiner le flux de prompt pour permettre l'intégration du modèle personnalisé.
...
1. Ajoutez le code suivant au fichier *integrate_with_promptflow.py* pour utiliser le modèle Phi-3 personnalisé dans le flux de prompt.
    ...
    def query_phi3_model(input_data: str, connection: CustomConnection) -> str:
        """
        Envoyer une requête à l'endpoint du modèle Phi-3 avec les données d'entrée données en utilisant la Connexion Personnalisée.
        """
    ...
    ![Coller le code du flux de prompt.](../../../../translated_images/08-16-paste-promptflow-code.c27a93ed6134adbe7ce618ffad7300923f3c02defedef0b5598eab5a6ee2afc2.fr.png)
...
1. Vous êtes maintenant prêt à discuter avec votre modèle Phi-3 personnalisé. Dans le prochain exercice, vous apprendrez comment démarrer le flux de prompt et l'utiliser pour discuter avec votre modèle Phi-3 ajusté.
...
Maintenant que vous avez ajusté et intégré votre modèle Phi-3 personnalisé avec Prompt flow, vous êtes prêt à commencer à interagir avec lui. Cet exercice vous guidera à travers le processus de configuration et d'initiation d'une conversation avec votre modèle en utilisant Prompt flow. En suivant ces étapes, vous pourrez pleinement utiliser les capacités de votre modèle Phi-3 ajusté pour diverses tâches et conversations.
...
1. Sélectionnez **Démarrer les sessions de calcul** pour démarrer le flux de prompt.
    ...
    ![Démarrer session de calcul.](../../../../translated_images/09-01-start-compute-session.9d080c30a6fc78a8b23ac54e7c8b11aeecc005d3da03cb0f9bd8afc298151ffa.fr.png)

1. Sélectionnez **Valider et analyser l'entrée** pour renouveler les paramètres.

    ![Valider entrée.](../../../../translated_images/09-02-validate-input.db05a40e29a21b333848b7c03542b0ec521ce9c6fbe12fba51c2addcb1c07c68.fr.png)
...
1. Voici un exemple des résultats : Maintenant, vous pouvez discuter avec votre modèle Phi-3 personnalisé. Il est recommandé de poser des questions basées sur les données utilisées pour l'ajustement.
    ...
    ![Chat avec flux de prompt.](../../../../translated_images/09-05-chat-with-promptflow.46c9fdf0e6de0e15e9618f654830e52bd8ead4aec0de57bb960206321d2bd0bd.fr.png)
