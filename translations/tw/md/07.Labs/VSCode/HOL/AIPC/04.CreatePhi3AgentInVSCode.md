# **Lab 3 - 在 GitHub Copilot Chat 中创建 @PHI3 agent**

在 Visual Studio Code Copilot Chat 中，你可以通过 @workspace 管理整个项目。对于企业场景，我们还可以更灵活地定制 Copilot Chat 的 Agent。

在上一个 Lab 中，我们结合 NPU 和云完成了一个 Phi3 Agent 的 Visual Studio Code 扩展。
...
***NOTE:*** 请使用 Typescript 和 WebPack 作为这个项目的技术方案

### **2. 添加 vscode.d.ts**

Visual Studio Code Chat API 还没有合并到 code API 中，但现在可以通过扩展添加。
...
**免責聲明**：
本文件使用基於機器的AI翻譯服務進行翻譯。我們努力確保準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。應以原始語言的文件為權威來源。對於關鍵信息，建議尋求專業人工翻譯。我們對使用此翻譯所產生的任何誤解或誤讀不承擔責任。