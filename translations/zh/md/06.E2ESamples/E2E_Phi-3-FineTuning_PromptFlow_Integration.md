...
这个端到端（E2E）示例基于 Microsoft Tech Community 的指南“[Fine-Tune and Integrate Custom Phi-3 Models with Prompt Flow: Step-by-Step Guide](https://techcommunity.microsoft.com/t5/educator-developer-blog/fine-tune-and-integrate-custom-phi-3-models-with-prompt-flow/ba-p/4178612?WT.mc_id=aiml-137032-kinfeylo)”。它介绍了如何使用 Prompt flow 微调、部署和集成自定义 Phi-3 模型的过程。

## 概述

在这个端到端示例中，你将学习如何微调 Phi-3 模型并将其与 Prompt flow 集成。通过利用 Azure Machine Learning 和 Prompt flow，你将建立一个部署和使用自定义 AI 模型的工作流程。这个端到端示例分为三个场景：
...
在这个端到端示例中，你将使用 *Standard_NC24ads_A100_v4 GPU* 进行微调，这需要请求配额，并使用 *Standard_E4s_v3* CPU 进行部署，这不需要请求配额。

> [!NOTE]
>
> 只有按需付费订阅（标准订阅类型）才有资格分配 GPU；福利订阅目前不支持。
>
> 对于使用福利订阅（如 Visual Studio 企业订阅）或希望快速测试微调和部署过程的用户，本教程还提供了使用 CPU 和最小数据集进行微调的指导。但需要注意的是，使用 GPU 和更大数据集进行微调的结果显著更好。
...
要微调和部署你的模型，你必须首先创建一个用户分配的托管身份（UAI）并赋予其适当的权限。这个 UAI 将在部署期间用于身份验证。
...
1. 选择与你的 Azure Machine Learning 工作区关联的容器注册表。例如，*finetunephicontainerregistries*。
...
现在，你将创建一个工作文件夹并设置一个虚拟环境，以开发一个与用户交互并使用 Azure Cosmos DB 存储的聊天记录来生成响应的程序。

#### 创建一个工作文件夹

1. 打开一个终端窗口，输入以下命令在默认路径中创建一个名为 *finetune-phi* 的文件夹。
...
在这个练习中，你将为项目创建必要的文件。这些文件包括下载数据集的脚本、设置 Azure Machine Learning 环境的脚本、微调 Phi-3 模型的脚本以及部署微调模型的脚本。你还将创建一个 *conda.yml* 文件来设置微调环境。

在这个练习中，你将：

- 创建一个 *download_dataset.py* 文件以下载数据集。
- 创建一个 *setup_ml.py* 文件以设置 Azure Machine Learning 环境。
- 在 *finetuning_dir* 文件夹中创建一个 *fine_tune.py* 文件以使用数据集微调 Phi-3 模型。
- 创建一个 *conda.yml* 文件来设置微调环境。
- 创建一个 *deploy_model.py* 文件以部署微调后的模型。
- 创建一个 *integrate_with_promptflow.py* 文件，以集成微调后的模型并使用 Prompt flow 执行模型。
- 创建一个 flow.dag.yml 文件，以设置 Prompt flow 的工作流程结构。
- 创建一个 *config.py* 文件以输入 Azure 信息。
...
1. 在 *conda.yml* 文件中添加以下代码，以设置 Phi-3 模型的微调环境。
...
1. 在 *config.py* 文件中添加以下代码，以包含你的 Azure 信息。
