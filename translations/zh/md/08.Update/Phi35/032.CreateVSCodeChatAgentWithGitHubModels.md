...
你是否在使用 Visual Studio Code Copilot？特别是在聊天功能中，你可以使用不同的代理来提升在 Visual Studio Code 中创建、编写和维护项目的能力。Visual Studio Code 提供了一个 API，允许公司和个人根据自己的业务创建不同的代理，以扩展其在不同专有领域的能力。本文将重点介绍 GitHub Models 的 **Phi-3.5-mini-instruct (128k)** 和 **Phi-3.5-vision-instruct (128k)** 来创建你自己的 Visual Studio Code Agent。

## **关于 GitHub Models 的 Phi-3.5**

我们知道 Phi-3/3.5 Family 中的 Phi-3/3.5-mini-instruct 具有强大的代码理解和生成能力，并且在某些方面优于 Gemma-2-9b 和 Mistral-Nemo-12B-instruct-2407。
...
***注意：*** 建议使用 Azure AI Inference SDK，因为它可以更好地在生产环境中与 Azure Model Catalog 进行切换。

以下是 **Phi-3.5-mini-instruct (128k)** 和 **Phi-3.5-vision-instruct (128k)** 在与 GitHub Models 对接后的代码生成场景中的结果，并为后续的示例做准备。

**演示：GitHub Models Phi-3.5-mini-instruct (128k) 从提示生成代码** ([点击此链接](../../../../../code/09.UpdateSamples/Aug/ghmodel_phi35_instruct_demo.ipynb))
...
GitHub Copilot Chat Agent 可以基于代码在不同的项目场景中完成不同的任务。系统有四个代理：workspace, github, terminal, vscode。

![agent](../../../../../translated_images/agent.0256841baa25f5bebbc7af293cdfdff45a5e641c8c61caf0a26b936642c77292.zh.png)

通过在代理名称前添加‘@’，你可以快速完成相应的工作。对于企业来说，如果添加与自己业务相关的内容，如需求、编码、测试规范和发布，可以基于 GitHub Copilot 拥有更强大的企业私有功能。

Visual Studio Code Chat Agent 现已正式发布其 API，允许企业或企业开发人员根据不同的软件业务生态系统开发代理。基于 Visual Studio Code 扩展开发的方法，你可以轻松访问 Visual Studio Code Chat Agent API 的接口。我们可以基于这个流程进行开发。

![diagram](../../../../../translated_images/diagram.43917a219cc91a95c00fea4dd3e33945ec2cb79a80125c6274acf49a93adecc9.zh.png)

开发场景可以支持接入第三方模型 API（如 GitHub Models, Azure Model Catalog 和基于开源模型自建的服务），也可以使用 GitHub Copilot 提供的 gpt-35-turbo, gpt-4 和 gpt-4o 模型。

## **基于 Phi-3.5 添加一个 Agent @phicoding**

我们尝试整合 Phi-3.5 的编程能力来完成代码编写、图像生成代码等任务。完成一个围绕 Phi-3.5 构建的 Agent - @PHI，以下是一些功能：

1. 通过 **@phicoding /help** 命令生成由 GitHub Copilot 提供的 GPT-4o 的自我介绍。
...
3. 打开创建的项目并修改 package.json。这里是相关的说明和配置，以及 GitHub Models 的配置。注意你需要在这里添加你的 GitHub Models 令牌。
...
**免责声明**：
本文档使用基于机器的人工智能翻译服务进行翻译。尽管我们力求准确，但请注意，自动翻译可能包含错误或不准确之处。应将原文档视为权威来源。对于关键信息，建议进行专业人工翻译。对于因使用本翻译而产生的任何误解或误读，我们不承担责任。