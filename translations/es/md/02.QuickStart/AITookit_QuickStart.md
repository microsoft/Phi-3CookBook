...
Para la inferencia o afinación local, dependiendo del modelo que selecciones, es posible que necesites tener una GPU como NVIDIA CUDA GPU.

Si ejecutas de forma remota, el recurso en la nube necesita tener GPU, asegúrate de verificar tu entorno. Para la ejecución local en Windows + WSL, la distribución de WSL Ubuntu 18.4 o superior debe estar instalada y configurada como predeterminada antes de usar AI Toolkit.
...
- Para afinar en Windows y Linux, necesitarás una GPU Nvidia. Además, **Windows** requiere el subsistema de Linux con la distribución de Ubuntu 18.4 o superior. [Aprende más sobre cómo instalar el subsistema de Windows para Linux](https://learn.microsoft.com/windows/wsl/install) y [cómo cambiar la distribución predeterminada](https://learn.microsoft.com/windows/wsl/install#change-the-default-linux-distribution-installed).
...
3. Selecciona "AI Toolkit for Visual Studio code"
...
- Encuentra un modelo compatible en **Catálogo de Modelos** y descárgalo localmente
- Prueba la inferencia del modelo en el **Playground de Modelos**
- Afina el modelo localmente o remotamente en **Afinación de Modelos**
- Despliega modelos afinados a la nube a través de la paleta de comandos de AI Toolkit

> [!NOTE]
>
> **GPU Vs CPU**
>
> Notarás que las tarjetas de los modelos muestran el tamaño del modelo, la plataforma y el tipo de acelerador (CPU, GPU). Para un rendimiento optimizado en **dispositivos Windows que tienen al menos una GPU**, selecciona versiones de modelos que solo apunten a Windows.
>
> Esto asegura que tengas un modelo optimizado para el acelerador DirectML.
>
> Los nombres de los modelos tienen el formato de
>
...
Cuando el modelo se haya descargado, puedes iniciar el proyecto desde AI Toolkit.

> ***Nota*** Si deseas probar la función de vista previa para hacer inferencia o afinación de forma remota, sigue [esta guía](https://aka.ms/previewFinetune)
...
![Flujo de generación](../../../../imgs/04/04/AItoolkitgeneration-gif.gif)

### Selecciones de Modelos
...
Deberías descargar en su lugar la versión optimizada para CPU:
...
- *Penalización por frecuencia*: Este parámetro influye en la frecuencia con la que el modelo repite palabras o frases en su salida. Un valor más alto (más cercano a 1.0) anima al modelo a *evitar* repetir palabras o frases.
...
AI Toolkit viene con un servidor web API REST local **en el puerto 5272** que utiliza el [formato de completaciones de chat de OpenAI](https://platform.openai.com/docs/api-reference/chat/create).
...
## Afinación con AI Toolkit

- Comienza con el descubrimiento de modelos y el playground.
- Afinación de modelos e inferencia usando recursos de computación locales.
- Afinación remota e inferencia usando recursos de Azure.

[Afinación con AI Toolkit](../04.Fine-tuning/Finetuning_VSCodeaitoolkit.md)

## Recursos de Q&A de AI Toolkit

Por favor, consulta nuestra [página de Q&A](https://github.com/microsoft/vscode-ai-toolkit/blob/main/QA.md) para los problemas más comunes y sus resoluciones.
