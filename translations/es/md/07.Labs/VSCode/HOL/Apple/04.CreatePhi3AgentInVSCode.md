...
En el último Lab, combinamos el framework MX y la nube para completar una Extensión de Visual Studio Code para el Agente Phi3.
...
***NOTE:*** Por favor, ejecútalo en Visual Studio Code Insiders 1.90+
...
export function activate(extcontext: vscode.ExtensionContext) {
	...
	const phi3handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<IPHI3ChatResult> => {
		...
		if (request.command == 'img') {
			...
			{
				var result = "Por favor, haz tu pregunta de esta manera: Tu pregunta (IMG_URL:https://example.com/image.jpg)";
