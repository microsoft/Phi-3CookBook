...
Importa el módulo os, que proporciona una forma portátil de usar funcionalidades dependientes del sistema operativo. Usa la función os.system para ejecutar el script download-dataset.py en la terminal con argumentos específicos de línea de comandos. Los argumentos especifican el conjunto de datos a descargar (HuggingFaceH4/ultrachat_200k), el directorio donde se descargará (ultrachat_200k_dataset), y el porcentaje del conjunto de datos a dividir (5). La función os.system devuelve el estado de salida del comando que ejecutó; este estado se almacena en la variable exit_status. Verifica si exit_status no es 0. En sistemas operativos tipo Unix, un estado de salida de 0 usualmente indica que un comando ha tenido éxito, mientras que cualquier otro número indica un error. Si exit_status no es 0, lanza una excepción con un mensaje indicando que hubo un error al descargar el conjunto de datos. En resumen, este script ejecuta un comando para descargar un conjunto de datos usando un script auxiliar, y lanza una excepción si el comando falla. ```
...
# Los argumentos especifican el conjunto de datos a descargar (HuggingFaceH4/ultrachat_200k), el directorio donde se descargará (ultrachat_200k_dataset), y el porcentaje del conjunto de datos a dividir (5)
...
# En sistemas operativos tipo Unix, un estado de salida de 0 usualmente indica que un comando ha tenido éxito, mientras que cualquier otro número indica un error
# Si exit_status no es 0, lanza una excepción con un mensaje indicando que hubo un error al descargar el conjunto de datos
if exit_status != 0:
...
Configura el ancho máximo de columna para las opciones de visualización de pandas a 0. Esto significa que el texto completo de cada columna se mostrará sin truncamiento cuando se imprima el DataFrame.
...
Crea el trabajo que utiliza el componente de la tubería chat-completion. Aprende más sobre todos los parámetros admitidos para el ajuste fino.

### Definir parámetros de ajuste fino

Los parámetros de ajuste fino pueden agruparse en 2 categorías: parámetros de entrenamiento, parámetros de optimización.

Los parámetros de entrenamiento definen los aspectos de entrenamiento tales como:
...
- Los parámetros de optimización ayudan a optimizar la memoria GPU y a utilizar eficazmente los recursos de cómputo.
...
**Nota:** El ajuste fino supervisado puede resultar en la pérdida de alineación o el olvido catastrófico. Recomendamos verificar este problema y ejecutar una etapa de alineación después de ajustar finamente.

### Parámetros de Ajuste Fino

Este script de Python configura los parámetros para el ajuste fino de un modelo de aprendizaje automático. Aquí tienes un desglose de lo que hace:

Configura parámetros de entrenamiento predeterminados como el número de épocas de entrenamiento, tamaños de lote para entrenamiento y evaluación, tasa de aprendizaje y tipo de programador de tasa de aprendizaje.

Configura parámetros de optimización predeterminados como si se aplicará Layer-wise Relevance Propagation (LoRa) y DeepSpeed, y la etapa de DeepSpeed.
...
Este script de Python define una función para generar un nombre de visualización para una tubería de entrenamiento de aprendizaje automático, y luego llama a esta función para generar e imprimir el nombre de visualización. Aquí tienes un desglose de lo que hace:

Se define la función get_pipeline_display_name. Esta función genera un nombre de visualización basado en varios parámetros relacionados con la tubería de entrenamiento.
...
Recupera varios otros parámetros como el tipo de programador de tasa de aprendizaje, si se aplica DeepSpeed, la etapa de DeepSpeed, si se aplica Layer-wise Relevance Propagation (LoRa), el límite en el número de puntos de control del modelo a mantener y la longitud máxima de la secuencia.
...
La función devuelve esta cadena, que sirve como el nombre de visualización para la tubería de entrenamiento.

Después de definir la función, se llama para generar el nombre de visualización, que luego se imprime.

En resumen, este script genera un nombre de visualización para una tubería de entrenamiento de aprendizaje automático basado en varios parámetros, y luego imprime este nombre de visualización.

```
# Define una función para generar un nombre de visualización para la tubería de entrenamiento
def get_pipeline_display_name():
    ...
    # Recupera el tipo de programador de tasa de aprendizaje
...
```
### Configurando la Tubería

Este script de Python define y configura una tubería de aprendizaje automático usando el SDK de Azure Machine Learning. Aquí tienes un desglose de lo que hace:

1. Importa los módulos necesarios del SDK de Azure AI ML.

2. Obtiene un componente de tubería llamado "chat_completion_pipeline" del registro.

3. Define un trabajo de tubería usando el decorador `@pipeline` y la función `create_pipeline`. El nombre de la tubería se establece en `pipeline_display_name`.

4. Dentro de la función `create_pipeline`, inicializa el componente de tubería obtenido con varios parámetros, incluidos el camino del modelo, los clústeres de cómputo para diferentes etapas, las divisiones del conjunto de datos para entrenamiento y prueba, el número de GPUs a usar para el ajuste fino y otros parámetros de ajuste fino.

5. Mapea la salida del trabajo de ajuste fino a la salida del trabajo de tubería. Esto se hace para que el modelo ajustado se pueda registrar fácilmente, lo cual es necesario para desplegar el modelo en un punto final en línea o por lotes.

6. Crea una instancia de la tubería llamando a la función `create_pipeline`.

7. Configura la opción `force_rerun` de la tubería a `True`, lo que significa que no se usarán resultados en caché de trabajos anteriores.

8. Configura la opción `continue_on_step_failure` de la tubería a `False`, lo que significa que la tubería se detendrá si algún paso falla.

En resumen, este script define y configura una tubería de aprendizaje automático para una tarea de completado de chat usando el SDK de Azure Machine Learning.
...
# Obtén el componente de tubería llamado "chat_completion_pipeline" del registro
pipeline_component_func = registry_ml_client.components.get(
    name="chat_completion_pipeline", label="latest"
)

# Define el trabajo de tubería usando el decorador @pipeline y la función create_pipeline
# El nombre de la tubería se establece en pipeline_display_name
@pipeline(name=pipeline_display_name)
def create_pipeline():
    # Inicializa el componente de tubería obtenido con varios parámetros
    # Estos incluyen el camino del modelo, los clústeres de cómputo para diferentes etapas, las divisiones del conjunto de datos para entrenamiento y prueba, el número de GPUs a usar para el ajuste fino, y otros parámetros de ajuste fino
    chat_completion_pipeline = pipeline_component_func(
        ...
        number_of_gpu_to_use_finetuning=gpus_per_node,  # Establece el número de GPUs disponibles en el cómputo
        **finetune_parameters
    )
    return {
        # Mapea la salida del trabajo de ajuste fino a la salida del trabajo de tubería
        # Esto se hace para que podamos registrar fácilmente el modelo ajustado
        # Registrar el modelo es necesario para desplegar el modelo en un punto final en línea o por lotes
        "trained_model": chat_completion_pipeline.outputs.mlflow_model_folder
    }

# Crea una instancia de la tubería llamando a la función create_pipeline
pipeline_object = create_pipeline()

# No uses resultados en caché de trabajos anteriores
pipeline_object.settings.force_rerun = True

# Configura continuar en caso de fallo de paso a False
# Esto significa que la tubería se detendrá si algún paso falla
pipeline_object.settings.continue_on_step_failure = False
```
### Enviar el Trabajo

Este script de Python envía un trabajo de tubería de aprendizaje automático a un espacio de trabajo de Azure Machine Learning y luego espera a que el trabajo se complete. Aquí tienes un desglose de lo que hace:

Llama al método create_or_update del objeto jobs en workspace_ml_client para enviar el trabajo de tubería. La tubería a ejecutar se especifica mediante pipeline_object, y el experimento bajo el cual se ejecuta el trabajo se especifica mediante experiment_name.

Luego llama al método stream del objeto jobs en workspace_ml_client para esperar a que el trabajo de tubería se complete. El trabajo a esperar se especifica mediante el atributo name del objeto pipeline_job.

En resumen, este script envía un trabajo de tubería de aprendizaje automático a un espacio de trabajo de Azure Machine Learning, y luego espera a que el trabajo se complete.

```
# Envía el trabajo de tubería al espacio de trabajo de Azure Machine Learning
# La tubería a ejecutar se especifica mediante pipeline_object
# El experimento bajo el cual se ejecuta el trabajo se especifica mediante experiment_name
pipeline_job = workspace_ml_client.jobs.create_or_update(
    pipeline_object, experiment_name=experiment_name
)

# Espera a que el trabajo de tubería se complete
...
Este script de Python registra un modelo de aprendizaje automático que fue entrenado en una tubería de Azure Machine Learning. Aquí tienes un desglose de lo que hace:

Importa los módulos necesarios del SDK de Azure AI ML.

Verifica si la salida trained_model está disponible desde el trabajo de tubería llamando al método get del objeto jobs en workspace_ml_client y accediendo a su atributo outputs.

Construye un camino al modelo entrenado formateando una cadena con el nombre del trabajo de tubería y el nombre de la salida ("
```
...
Los endpoints en línea proporcionan una API REST duradera que se puede utilizar para integrar con aplicaciones que necesitan usar el modelo.
...
endpoint = ManagedOnlineEndpoint(
    ...
    description="Endpoint en línea para "
    + registered_model.name
    + ", modelo ajustado para ultrachat-200k-chat-completion",
    auth_mode="key",
)
...
if "inference_compute_allow_list" in foundation_model.tags:
    ...
    print(f"Por favor, crea un compute de la lista anterior - {computes_allow_list}")
else:
    # Si no lo está, establecer `inference_computes_allow_list` en `None`
    inference_computes_allow_list = None
    print("`inference_compute_allow_list` no es parte de las etiquetas del modelo")
...
if (
    ...
    print(
        f"`instance_type` no está en la lista permitida de computes. Por favor, selecciona un valor de {inference_computes_allow_list}"
    )
...
Muestra las primeras 2 filas del DataFrame usando la función head con el argumento 2. Sin embargo, como el DataFrame solo contiene una fila después de la muestra, esto solo mostrará esa fila.
...
# El argumento 'inplace=True' indica que el DataFrame debe ser modificado in situ (sin crear un nuevo objeto)
test_df.reset_index(drop=True, inplace=True)

# Mostrar las primeras 2 filas del DataFrame
# Sin embargo, como el DataFrame solo contiene una fila después de la muestra, esto solo mostrará esa fila
test_df.head(2)
```
### Crear Objeto JSON
...
# El método `invoke` de la propiedad `online_endpoints` del objeto `workspace_ml_client` se utiliza para enviar una solicitud a un endpoint en línea y obtener una respuesta
...
No olvides eliminar el endpoint en línea, de lo contrario, dejarás el medidor de facturación en funcionamiento para el compute utilizado por el endpoint. Esta línea de código de Python está eliminando un endpoint en línea en Azure Machine Learning. Aquí tienes un desglose de lo que hace:
...
Llama al método wait para esperar a que se complete la operación de eliminación. Esta es una operación de bloqueo, lo que significa que evitará que el script continúe hasta que la eliminación haya finalizado.

En resumen, esta línea de código está iniciando la eliminación de un endpoint en línea en Azure Machine Learning y esperando a que se complete la operación.

```
# Eliminar el endpoint en línea en Azure Machine Learning
# El método `begin_delete` de la propiedad `online_endpoints` del objeto `workspace_ml_client` se utiliza para iniciar la eliminación de un endpoint en línea
# El argumento `name` especifica el nombre del endpoint a eliminar, que se almacena en la variable `online_endpoint_name`
# El método `wait` se llama para esperar a que se complete la operación de eliminación. Esta es una operación de bloqueo, lo que significa que evitará que el script continúe hasta que la eliminación haya finalizado
