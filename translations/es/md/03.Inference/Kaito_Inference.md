...
- Gestionar archivos de modelos utilizando imágenes de contenedores. Se proporciona un servidor http para realizar llamadas de inferencia utilizando la biblioteca del modelo.
...
- Hospedar imágenes de modelos grandes en el Registro de Contenedores Público de Microsoft (MCR) si la licencia lo permite.
...
- **Controlador de Workspace**: Concilia el recurso personalizado `workspace`, crea recursos personalizados `machine` (explicado abajo) para activar el aprovisionamiento automático de nodos, y crea la carga de trabajo de inferencia (`deployment` o `statefulset`) basada en las configuraciones preestablecidas del modelo.
...
> Note: El [*gpu-provisioner*](https://github.com/Azure/gpu-provisioner) es un componente de código abierto. Puede ser reemplazado por otros controladores si soportan las API de [Karpenter-core](https://sigs.k8s.io/karpenter).
...
inference:
  preset:
    ...
    # Note: Esta configuración también funciona con el preset phi-3-mini-128k-instruct
```
...
El estado del workspace se puede rastrear ejecutando el siguiente comando. Cuando la columna WORKSPACEREADY se convierte en `True`, el modelo se ha desplegado con éxito.
