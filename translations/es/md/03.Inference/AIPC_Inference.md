...
Con el avance de la IA generativa y la mejora en las capacidades de hardware de los dispositivos edge, un número creciente de modelos de IA generativa ahora puede integrarse en los dispositivos Bring Your Own Device (BYOD) de los usuarios. Los AI PCs son uno de estos modelos. A partir de 2024, Intel, AMD y Qualcomm han colaborado con fabricantes de PCs para introducir AI PCs que faciliten el despliegue de modelos de IA generativa localizados a través de modificaciones de hardware. En esta discusión, nos centraremos en los AI PCs de Intel y exploraremos cómo desplegar Phi-3 en un AI PC de Intel.

### Qué es un NPU

Un NPU (Unidad de Procesamiento Neural) es un procesador o unidad de procesamiento dedicada en un SoC más grande, diseñada específicamente para acelerar las operaciones de redes neuronales y tareas de IA. A diferencia de las CPUs y GPUs de propósito general, los NPUs están optimizados para una computación paralela basada en datos, lo que los hace altamente eficientes en el procesamiento de datos multimedia masivos como videos e imágenes y en el procesamiento de datos para redes neuronales. Son particularmente hábiles en manejar tareas relacionadas con la IA, como el reconocimiento de voz, el desenfoque de fondo en videollamadas y procesos de edición de fotos o videos como la detección de objetos.
...
Las GPUs son conocidas por sus capacidades de computación paralela, pero no todas las GPUs son igualmente eficientes más allá del procesamiento gráfico. Los NPUs, por otro lado, están construidos específicamente para los cálculos complejos involucrados en las operaciones de redes neuronales, lo que los hace altamente efectivos para tareas de IA.

En resumen, los NPUs son los genios matemáticos que impulsan los cálculos de IA, ¡y juegan un papel clave en la emergente era de los AI PCs!

***Este ejemplo se basa en el último procesador Intel Core Ultra***

## **1. Usar NPU para ejecutar el modelo Phi-3**

El dispositivo Intel® NPU es un acelerador de inferencia de IA integrado con CPUs cliente de Intel, comenzando desde la generación de CPUs Intel® Core™ Ultra (anteriormente conocida como Meteor Lake). Permite la ejecución eficiente en términos de energía de tareas de redes neuronales artificiales.
...
Ejemplo de Phi-3-mini en un AI PC impulsado por procesadores Intel® Core™ Ultra.
...
Usando la aceleración de Intel NPU, esta biblioteca no afecta el proceso de codificación tradicional. Solo necesitas usar esta biblioteca para cuantizar el modelo original de Phi-3, como FP16, INT8, INT4, por ejemplo
...
```
Una vez que la cuantificación es exitosa, continúa la ejecución para llamar al NPU y ejecutar el modelo Phi-3.
...
Al ejecutar el código, podemos ver el estado de funcionamiento del NPU a través del Administrador de Tareas.
...
- Basado en DX12: DirectML está construido sobre DirectX 12 (DX12), lo que proporciona una amplia gama de soporte de hardware a través de GPUs, incluidas tanto NVIDIA como AMD.
...
- Procesamiento de imágenes: DirectML procesa imágenes y otros datos usando redes neuronales, lo que lo hace adecuado para tareas como el reconocimiento de imágenes, la detección de objetos y más.
...
- Limitaciones: Sin embargo, la dependencia de CUDA en el hardware de NVIDIA puede ser limitante si deseas una compatibilidad más amplia a través de diferentes GPUs.
...
Al ejecutar el código, podemos ver el estado de funcionamiento del GPU a través del Administrador de Tareas
